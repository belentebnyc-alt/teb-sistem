...
EDICIONES = {
    'Julio 2026': {
        ...
        'fin':      date(2026, 7, 31),
        'checkin':  '10 de julio de 2026 a las 4:00 PM',
        'checkout': '31 de julio de 2026 a las 12:00 PM',
        'label':    'TEB NYC — Julio 2026',
        'duracion': '21 dias',
    },
    'Enero 2027': {
        'inicio':   date(2027, 1, 27),
        'fin':      date(2027, 2, 10),
        'checkin':  '27 de enero de 2027 a las 4:00 PM',
        'checkout': '10 de febrero de 2027 a las 12:00 PM',
        'label':    'TEB NYC — Enero 2027',
        'duracion': '14 dias',
    },
    'Julio 2027': {
        'inicio':   date(2027, 7, 16),
        'fin':      date(2027, 7, 30),
        'checkin':  '16 de julio de 2027 a las 4:00 PM',
        'checkout': '30 de julio de 2027 a las 12:00 PM',
        'label':    'TEB NYC — Julio 2027',
        'duracion': '14 dias',
    },
    'Enero 2028': {
        'inicio':   date(2028, 1, 15),
        'fin':      date(2028, 1, 31),
        'checkin':  'A confirmar',
        'checkout': 'A confirmar',
        'label':    'TEB NYC — Enero 2028',
...
def calcular_cuotas(fecha_viaje, total, n):
    ...
    # Buscar el dia 15 del mes que no supere la fecha limite
    ...
    return [{'n': i+1, 'fecha': f, 'monto': (ult if i==n-1 else base), 'ultima': i==n-1}
            for i, f in enumerate(fechas)], candidato
...
def estilos():
    return {
        ...
        'nor_b':  ParagraphStyle('nb',   fontName='Helvetica-Bold',   fontSize=10, textColor=colors.HexColor('#1A1A1A')),
        ...
        'plan_t': ParagraphStyle('pt',   fontName='Helvetica-Bold',   fontSize=11, textColor=V_OSC),
        'plan_d': ParagraphStyle('pd',   fontName='Helvetica',        fontSize=9,  textColor=G_TXT, leading=12),
        'small':  ParagraphStyle('sm',   fontName='Helvetica',        fontSize=8,  textColor=G_TXT, leading=11),
    }
...
def generar_pdf(datos):
    ...
    at.setStyle(TableStyle([
        ('BACKGROUND',   (0,0),(-1,-1), G_CLA),
        ('TOPPADDING',   (0,0),(-1,-1), 8),
        ('BOTTOMPADDING',(0,0),(-1,-1), 8),
        ('LEFTPADDING',  (0,0),(-1,-1), 12),
        ('LINEBELOW',    (0,0),(-1,0),  0.5, G_LIN),
        ('BOX',          (0,0),(-1,-1), 0.5, G_LIN),
    ]))
    st.append(at)
    ...
    if ed_key in EDICIONES:
        ...
        fecha_rows = [
            ...
            [Paragraph(ed_data['checkin'], E['nor']),
             Paragraph(ed_data['checkout'], E['nor']),
             Paragraph(ed_data['duracion'], E['val'])],
        ]
        ft = Table(fecha_rows, colWidths=[W*0.42, W*0.42, W*0.16])
        ft.setStyle(TableStyle([
            ('BACKGROUND',   (0,0),(-1,-1), colors.HexColor('#E6F1FB')),
            ('BACKGROUND',   (0,0),(-1,0),  colors.HexColor('#B5D4F4')),
            ('TOPPADDING',   (0,0),(-1,-1), 8),
            ('BOTTOMPADDING',(0,0),(-1,-1), 8),
            ('LEFTPADDING',  (0,0),(-1,-1), 12),
            ('BOX',          (0,0),(-1,-1), 0.5, colors.HexColor('#B5D4F4')),
            ('INNERGRID',    (0,0),(-1,-1), 0.3, colors.HexColor('#B5D4F4')),
        ]))
        st.append(ft)
        st.append(Spacer(1, 4*mm))
    ...
    p_bg   = plan_info['color']
    p_txt  = plan_info['color_txt']
    p_icono= plan_info['icono']
    ...
    pt = Table([
        [Paragraph(f"{p_icono}  {plan_key}", plan_label),
         Paragraph(PLAN_DESC.get(plan_key, ''), plan_desc_s)],
    ...
    eco = [
        ...
        [Paragraph('Valor del programa', E['nor']), Paragraph('', E['nor']),
         Paragraph(f'USD {valor:,.0f}', E['nor'])],
    ]
    if desc > 0:
        eco.append([Paragraph('Beca / descuento aplicado', E['nor']), Paragraph('', E['nor']),
                    Paragraph(f'- USD {desc:,.0f}',
                              ParagraphStyle('d', fontName='Helvetica', fontSize=10, textColor=V_MED))])
    if upgrade > 0:
        eco.append([Paragraph('Upgrade de plan', E['nor']), Paragraph('', E['nor']),
                    Paragraph(f'+ USD {upgrade:,.0f}',
                              ParagraphStyle('u', fontName='Helvetica', fontSize=10, textColor=AZUL))])
    if reprog > 0:
        eco.append([Paragraph('Costo de reprogramacion', E['nor']), Paragraph('', E['nor']),
                    Paragraph(f'+ USD {reprog:,.0f}',
                              ParagraphStyle('r', fontName='Helvetica', fontSize=10, textColor=ROJO))])
    eco.append(['', '', ''])  # separador
    eco.append([
        Paragraph('TOTAL A ABONAR', ParagraphStyle('tl', fontName='Helvetica-Bold', fontSize=11, textColor=V_OSC)),
        Paragraph('', E['nor']),
        Paragraph(f'USD {total:,.0f}',
                  ParagraphStyle('tv', fontName='Helvetica-Bold', fontSize=15, textColor=V_OSC, alignment=TA_RIGHT)),
    ])
    nf = len(eco)
    ...
    et.setStyle(TableStyle([
        ...
        ('LINEBELOW',    (0,0),(-1,0),   0.5, G_LIN),
        ('LINEABOVE',    (0,nf-1),(-1,nf-1), 1, V_BOR),
        ('BACKGROUND',   (0,nf-1),(-1,nf-1), V_CLA),
    ...
    resumen = (f"{n_cuotas} cuota{'s' if n_cuotas>1 else ''} de USD {cuotas[0]['monto']:,.0f}  "
               f"({fmt_mes(cuotas[0]['fecha'])} a {fmt_mes(cuotas[-1]['fecha'])})  "
               f"·  Pagos del 1 al 15 de cada mes")
    st.append(Paragraph(resumen, E['nor']))
    st.append(Spacer(1, 3*mm))

    ch = [Paragraph('#', E['cn']), Paragraph('Periodo', E['cn']),
          Paragraph('Vencimiento', E['cn']), Paragraph('Importe', E['cn']),
          Paragraph('Estado', E['cn'])]
    cr = [ch]
    for c in cuotas:
        ...
        cr.append([
            Paragraph(str(c['n']),
                      ParagraphStyle('x', fontName='Helvetica-Bold', fontSize=9,
                                     alignment=TA_CENTER, textColor=V_OSC)),
            Paragraph(fmt_mes(c['fecha']), E['ct']),
            Paragraph(f"Del 1 al 15 de {MESES[c['fecha'].month]}", E['ct']),
            Paragraph(f"USD {c['monto']:,.0f}", E['cm']),
            Paragraph(estado_txt,
                      ParagraphStyle('est', fontName='Helvetica' + ('-Bold' if c['ultima'] else ''),
                                     fontSize=8, textColor=estado_col, alignment=TA_CENTER)),
        ])

    ct = Table(cr, colWidths=[W*0.07, W*0.21, W*0.31, W*0.20, W*0.21], repeatRows=1)
    n_cuotas_tabla = len(cuotas)
    ct.setStyle(TableStyle([
        ('BACKGROUND',    (0,0),(-1,0),  V_OSC),
        ('TOPPADDING',    (0,0),(-1,0),  8),
        ('BOTTOMPADDING', (0,0),(-1,0),  8),
        ('ROWBACKGROUNDS',(0,1),(-1,-1), [BLANCO, G_CLA]),
        ...
        ('BOX',           (0,0),(-1,-1), 0.5, G_LIN),
        ('INNERGRID',     (0,1),(-1,-1), 0.3, G_LIN),
        # Última cuota destacada
        ('BACKGROUND',    (0,n_cuotas_tabla),(-1,n_cuotas_tabla), BCA),
        ('LINEABOVE',     (0,n_cuotas_tabla),(-1,n_cuotas_tabla), 1, BCA_B),
    ]))
    st.append(ct)
    st.append(Spacer(1, 3*mm))

    # ── ALERTA FECHA LÍMITE ────────────────────────────────────────────────────
    alerta_txt = (f"IMPORTANTE: El ultimo pago debe acreditarse antes del {fmt(fecha_lim)}. "
                  f"El viaje inicia el {fmt(fv)} — se requieren 45 dias de anticipacion.")
    ...
    alt.setStyle(TableStyle([
        ...
        ('BOX',          (0,0),(-1,-1), 0.5, colors.HexColor('#F7C1C1')),
    ]))
    st.append(alt)
    st.append(Spacer(1, 4*mm))
    ...
    mt = Table([[
        Paragraph('Stripe (tarjeta)', E['nor']),
        Paragraph('Transferencia bancaria', E['nor']),
        Paragraph('MercadoPago', E['nor']),
        Paragraph('BOFA / Wire transfer', E['nor']),
    ]], colWidths=[W/4]*4)
    ...
    st.append(Paragraph(
        f"Propuesta emitida el {fmt(date.today())} por {quien}  ·  TEB NYC  "
        f"·  Importes en dolares estadounidenses (USD)  ·  Validez: 15 dias.",
        E['pie']
    ))
...
if __name__ == '__main__':
    datos = {
        ...
        'reprogramacion': 0,
        'cuotas':        8,
        'quien_recibe':  'TEB NYC',
        'notas':         'Para confirmar la inscripcion se requiere abonar la primera cuota. Los pagos pueden realizarse en USD o su equivalente en moneda local al tipo de cambio del dia.',
    }
    pdf_bytes = generar_pdf(datos)
    with open('/mnt/user-data/outputs/cotizacion_teb_v2.pdf', 'wb') as f:
        f.write(pdf_bytes)
    print("PDF generado OK")

    # Segundo ejemplo: Comfort Stay+
    datos2 = {
        'nombre':        'Emmanuel Schmitt Guiñazú',
        'programa':      'Músico',
        'edicion':       'Julio 2027',
        'plan':          'Comfort Stay+',
        'valor_programa': 2760,
    ...
    pdf2 = generar_pdf(datos2)
    with open('/mnt/user-data/outputs/cotizacion_teb_v2b.pdf', 'wb') as f:
        f.write(pdf2)
    print("PDF 2 generado OK")
