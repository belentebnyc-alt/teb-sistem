...
EDICIONES = {
    'Julio 2026': {'inicio': date(2026,7,10), 'label':'TEB NYC - Julio 2026'},
    'Enero 2027': {'inicio': date(2027,1,27), 'label':'TEB NYC - Enero 2027'},
    'Julio 2027': {'inicio': date(2027,7,16), 'label':'TEB NYC - Julio 2027'},
    'Enero 2028': {'inicio': date(2028,1,15), 'label':'TEB NYC - Enero 2028'},
    'Julio 2028': {'inicio': date(2028,7,15), 'label':'TEB NYC - Julio 2028'},
    'Enero 2029': {'inicio': date(2029,1,15), 'label':'TEB NYC - Enero 2029'},
    'Julio 2029': {'inicio': date(2029,7,15), 'label':'TEB NYC - Julio 2029'},
}
...
PLAN_DESC = {
    ...
    'Sin hotel':     'El alumno gestiona su propio alojamiento.',
}
...
def fmt(d):
    return str(d.day) + ' de ' + MESES[d.month] + ' de ' + str(d.year)


def fmt_mes(d):
    return MESES[d.month].capitalize() + ' ' + str(d.year)
...
def generar_pdf(datos):
    ...
    n_cuotas = int(datos.get('cuotas', 1))
    ...
    if cuotas_fechas_str:
        ...
        cuotas = []
        for i, f in enumerate(fechas_manual):
            cuotas.append({
                'n': i + 1,
                'fecha': f,
                'monto': ult if i == n_cuotas - 1 else base,
                'ultima': i == n_cuotas - 1
            })
    ...
    eco = [
        ...
        [Paragraph('Valor del programa', E['nor']), '', Paragraph('USD ' + str(int(valor)), E['nor'])],
    ]
    if desc > 0:
        eco.append([Paragraph('Beca / descuento', E['nor']), '', Paragraph('- USD ' + str(int(desc)), ParagraphStyle('d', fontName='Helvetica', fontSize=10, textColor=GRIS2))])
    if desc5 > 0:
        eco.append([Paragraph('Descuento 5% Enero 2027', E['nor']), '', Paragraph('- USD ' + str(int(desc5)), ParagraphStyle('d5', fontName='Helvetica', fontSize=10, textColor=GRIS2))])
    if upgrade > 0:
        eco.append([Paragraph('Upgrade de hospedaje', E['nor']), '', Paragraph('+ USD ' + str(int(upgrade)), ParagraphStyle('u', fontName='Helvetica', fontSize=10, textColor=GRIS1))])
    if upgrade < 0:
        eco.append([Paragraph('Descuento sin hotel', E['nor']), '', Paragraph('- USD ' + str(int(abs(upgrade))), ParagraphStyle('sh', fontName='Helvetica', fontSize=10, textColor=GRIS2))])
    if reprog > 0:
        eco.append([Paragraph('Costo de reprogramacion', E['nor']), '', Paragraph('+ USD ' + str(int(reprog)), ParagraphStyle('r', fontName='Helvetica', fontSize=10, textColor=ROJO))])
    eco.append(['', '', ''])
    eco.append([
        Paragraph('TOTAL A ABONAR', ParagraphStyle('tl', fontName='Helvetica-Bold', fontSize=11, textColor=NEGRO)),
        '',
        Paragraph('USD ' + str(int(total)), ParagraphStyle('tv', fontName='Helvetica-Bold', fontSize=15, textColor=NEGRO, alignment=TA_RIGHT)),
    ])
    nf = len(eco)
    ...
    rango       = primer_mes if n_cuotas == 1 else primer_mes + '  a  ' + ultimo_mes
    monto_cuota = cuotas[0]['monto']

    plan_rows = []
    if pago_inicial_400:
        # Mes del pago inicial = mes anterior a la primera cuota
        primera = cuotas[0]['fecha']
        m_ini = primera.month - 1 if primera.month > 1 else 12
        y_ini = primera.year if primera.month > 1 else primera.year - 1
        mes_ini_str = MESES[m_ini].capitalize() + ' ' + str(y_ini)
        rango_cuotas = primer_mes if n_cuotas == 1 else primer_mes + '  a  ' + ultimo_mes
        plan_rows.append([Paragraph('Pago inicial:  USD 400  -  ' + mes_ini_str, E['plan_s'])])
        plan_rows.append([Paragraph(' ', ParagraphStyle('sp', fontSize=4))])
        plan_rows.append([Paragraph(str(n_cuotas) + (' cuota' if n_cuotas == 1 else ' cuotas') + '  de  USD ' + str(int(monto_cuota)), E['plan_n'])])
        plan_rows.append([Paragraph(rango_cuotas, E['plan_s'])])
        plan_rows.append([Paragraph('Del 1 al 15 de cada mes  -  Ultimo pago: antes del ' + fmt(fecha_lim), E['plan_d'])])
    else:
        plan_rows.append([Paragraph(str(n_cuotas) + (' pago unico' if n_cuotas == 1 else ' cuotas') + '  de  USD ' + str(int(monto_cuota)), E['plan_n'])])
        plan_rows.append([Paragraph(rango, E['plan_s'])])
        plan_rows.append([Paragraph('Del 1 al 15 de cada mes  -  Ultimo pago: antes del ' + fmt(fecha_lim), E['plan_d'])])

    plan_t = Table(plan_rows, colWidths=[W])
    ...
    alerta_txt = 'IMPORTANTE: El ultimo pago debe realizarse antes del ' + fmt(fecha_lim) + '.'
    ...
    st.append(Paragraph(
        'Propuesta emitida el ' + fmt(date.today()) + ' por ' + quien + '  -  TEB NYC  -  Importes en dolares estadounidenses (USD)  -  Validez: 15 dias.',
        E['pie']
    ))
    st.append(Spacer(1, 2*mm))
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        try:
            ...
            filename = 'TEB_' + nombre + '_' + edicion + '.pdf'
